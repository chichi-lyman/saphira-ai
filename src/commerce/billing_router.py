    ...
    try:
        async with conn.transaction():
            ...
            await conn.execute(
                ...
                subscription_id or f"session_{session.get('id')}", tenant_id, email, customer_id,
            )
        return tenant_id, True
    finally:
        await conn.close()
...
async def stripe_webhook(
    ...
    customer_id = str(obj.get('customer') or (obj.get('customer_details') or {}).get('email') or event_id)
    ...
    tenant_id, is_new = await _provision_subscription(obj)
